...
public class DrivingSchoolService {
	...
	public DrivingSchoolVo selectOne(Integer id, Long userId) throws Exception {
		...
		List<DrivingSchool> dataList = drivingSchoolMapper.selectByExample(example);

		if (null != dataList && dataList.size() > 0) {
			DrivingSchool data = dataList.get(0);
...
//		}
	 ...
	 * @throws Exception
	 */
	public Page<DrivingSchoolVo> selectConditionByPage(Integer areaId, Integer priceFrom, Integer priceTo,
			String licence, Integer pageIndex, Integer pageSize) throws Exception {
		Page<DrivingSchoolVo> result = new Page<DrivingSchoolVo>();
		// 单表分页
		...
		Page<DrivingSchool> pageList = (Page<DrivingSchool>)drivingSchoolMapper.selectByExample(example);
	 ...
	 * @throws Exception
	 */
	public Page<DrivingSchoolVo> selectConditionByPage2(Integer areaId, Integer priceFrom, Integer priceTo,
			String schoolName, String schoolAddress, Integer pageIndex, Integer pageSize) throws Exception {
		Page<DrivingSchoolVo> result = new Page<DrivingSchoolVo>();
		// 单表分页
		PageHelper.startPage(pageIndex, pageSize, true);
		...
		if (StringUtils.isNotEmpty(schoolName)) {
			criteria.andLike("nameL", "%" + schoolName + "%");
		}
		if (StringUtils.isNotEmpty(schoolAddress)) {
			criteria.andLike("address", "%" + schoolAddress + "%");
		}
     ...
     * @throws Exception
     */
	public Page<SchoolLessonVo> selectSchoolLessionByPage( Map<String,String> params
			) throws Exception {
		...
		if (schoolId != null) {
	     ...
	     example.setOrderByClause("update_time desc");
		Page<DrivingSchoolLesson> pageList = (Page<DrivingSchoolLesson>)drivingSchoolLessonMapper.selectByExample(example);
		Page<SchoolLessonVo> resultPageList  = new Page<SchoolLessonVo>();
		//统计报名人数
...
//			List<UserCourse> userCoursePageList = userCourseMapper.selectByExample(example);
    ...
    public int increaseCount(DrivingSchoolLesson model) {
    	...
    	int leftNum = model.getLeftNum() == null ? 0 : model.getLeftNum();
    	model.setLeftNum(leftNum + 1);
    	model.setUpdateTime(new Date());
    	return drivingSchoolLessonMapper.updateByPrimaryKey(model);   	
    }
